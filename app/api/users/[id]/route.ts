...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (body.role || body.employeeId !== undefined || body.fullName !== undefined || body.isActive !== undefined) {
    db.prepare(`UPDATE users SET
      ...
      WHERE id = ?`
    ).run(body.role || null, body.employeeId || null, body.fullName || null, body.isActive !== undefined ? (body.isActive ? 1 : 0) : null, params.id)
    if (body.role) changes.role = body.role
    if (body.isActive !== undefined) changes.isActive = body.isActive
  }

  logAudit(db, user.username, 'user.update', 'user', params.id, changes, getIp(req))
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const db = getDb()
  const target = db.prepare('SELECT * FROM users WHERE id = ?').get(params.id) as { id: number; username: string; role: string } | undefined
  ...
  logAudit(db, user.username, 'user.delete', 'user', params.id, { deletedUsername: target.username, role: target.role }, getIp(req))
  return NextResponse.json({ success: true })
}
