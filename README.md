...
- **💰 ระบบการเงิน — เอา OD ออก** — ลบระบบบัญชี OD (Overdraft) ออกจาก Sidebar, Finance Dashboard, และ API ทั้งหมด เพื่อให้ระบบกะทัดรัดขึ้น
- **📅 Format วันที่การเงิน DD-MM-YYYY** — ตาราง "รายรับ" และ "รายจ่าย" แสดงวันที่เป็นรูปแบบ DD-MM-YYYY (เช่น 01-03-2026) แทน YYYY-MM-DD เดิม
- **📅 แก้ไขวันที่ย้ายเดือนได้ถูกต้อง** — เมื่อแก้ไข entry_date ของรายรับ/รายจ่ายให้ข้ามเดือน ระบบจะอัปเดต year/month ใน database ให้ตรงกับวันที่ใหม่ทันที (ทั้ง front-end และ API PATCH routes)
- **🔃 Sort ตารางรายรับ / รายจ่าย** — คลิกหัวตาราง "วันที่", "ประเภท", "ยอดเงิน" เพื่อเรียงลำดับได้ทั้ง ASC/DESC พร้อม icon ▲▼ แสดงสถานะ
- **🏷️ เปลี่ยนชื่อรายรับ** — "🖨️ ยอดส่งพิมพ์" เปลี่ยนเป็น "ค่างานพิมพ์" / ลบคอลัมน์ "จำนวน × ราคา" ที่ไม่ได้ใช้งานออก / จัดลำดับคอลัมน์ใหม่: วันที่ → ประเภท → ยอดเงิน → รายละเอียด → จัดการ
- **💳 รายการปรับเงินเดือนหลายรายการ** — หน้าเงินเดือนรองรับการเพิ่มรายการ bonus/deduction ได้หลายรายการต่อ 1 คน โดยสร้างตาราง `payroll_adjustments` ใหม่ แทน field `extra_bonus`/`extra_deduction` เดิมที่รับได้แค่รายการเดียว พร้อม auto-migrate ข้อมูลเก่าอัตโนมัติ
- **👤 User — รายละเอียดปรับเงิน** — พนักงานในหน้า `/me/payroll` เห็นรายการปรับเงินพร้อม หมายเหตุ ของแต่ละรายการแยกแถวได้ชัดเจน
- **📊 Dashboard ผลผลิต — Label เดือน** — Card "ผลงานต่อพนักงาน" แสดงเดือน/วันที่ที่กำลังดูอยู่ต่อท้ายชื่อ Card ทุก view mode (รายวัน / รายสัปดาห์ / รายเดือน)
- **🖼️ Finance Dashboard — จัด Card กึ่งกลาง** — Stat cards และ Quick-link cards ในหน้าภาพรวมการเงินจัดกึ่งกลางหน้าจอด้วย flexbox
